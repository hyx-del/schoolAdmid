 ...
 * @LastEditors: 刘帝君
 * @LastEditTime: 2021-11-04 16:29:47
 * @Descripttion: 咨询课程
-->
<template>
  <div>
...
import {
  getConsultingCourseListAPI,
  changeStateItemAPI,
  addConsultingCourseAPI,
  editConsultingCourseAPI,
  deleteItemAPI,
  addStudyGoalAPI,
  editStudyGoalAPI
} from "@/api/baseSet";
...
export default {
  ...
  setup() {
    ...
    const getList = computed(() => {
      return getConsultingCourseListAPI;
    });
    const buttonCallBack = val => {
      if (val.name == "add") {
        // 弹窗 todo
        (state.dialogTitle = "添加咨询课程"), (state.dialogFormVisible = true);
