 ...
 * @LastEditors: 刘帝君
 * @LastEditTime: 2021-11-04 16:29:11
 * @Descripttion: 支付类型
-->
<template>
  <div>
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      ...
      popFun: computed(() =>
        state.dialogTitle == "添加支付方式" ? addPayTypeAPI : editPayTypeAPI
      ),
      column: [
        { prop: "label", label: "名称" },
        {
    ...
    const addPayType = () => {
      if (state.dialogTitle == "添加支付方式") {
        addPayTypeAPI({ ...state.form }).then(res => {
          if (res.code == 200) {
            //
            state.dialogTitle = "";
            state.form.label = "";
        ...
        editPayTypeAPI({ ...state.form }).then(res => {
          if (res.code == 200) {
            //
            state.dialogTitle = "";
            state.form.label = "";
