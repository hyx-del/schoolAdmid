 ...
 * @LastEditors: 刘帝君
 * @LastEditTime: 2021-11-24 14:49:57
 * @Descripttion:
-->
<template>
  <!-- <emptyBox bottom="70"> -->
          ...
          <div class="studentDetaileRight">
            <el-tabs v-model="activeName">
              <el-tab-pane label="基本信息" name="first">
                <BasicInfo v-if="activeName === 'first'" />
              </el-tab-pane>
              <el-tab-pane
                label="报名记录"
                name="second"
...
export default {
  ...
  setup() {
    ...
    console.log(store.getters.studentsPermissions, "getters按钮权限");
    ...
    // 页面载入后
    onMounted(() => {
      document.getElementsByClassName("main_layout_content")[0].scrollTop = 0;
      // 回显
      id && getStudentDetail();
    });
    // 学员详情
    ...
    const adjustChange = val => {
      state.adjustData[val.code] = val.value;
      switch (val.code) {
        case "type":
          state.adjustData = {
            ...state.adjustData,
          ...
          if (+val.value) {
            ...
            // 增加
            state.formState = [{ code: "money", disabled: false }];
          } else {
            // 减少
              ...
              state.formState = [
                {
                  code: "money",
                  disabled: false,
                  maxCode: +state.formInitData.balance
                }
              ];
            } else {
              // 更改提示语
